...
import ConflictModal, { FavoriteRecord, ConflictArtist } from '@/components/ConflictModal'
...
export default function MySetTimePage() {
    ...
    async function fetchFavorites() {
      try {
        ...
        const { data, error } = await supabase
          ...
          .eq('user_id', user.id)
        if (error) console.error('[my-settime] fetch error:', error)
        if (data) {
  ...
  const handleConflictSave = useCallback(
    async (aId: number, customEndA: string | null, bId: number, customStartB: string | null) => {
      const supabase = createClient()
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) return

      const [{ error: e1 }, { error: e2 }] = await Promise.all([
        supabase.schema('ultra').from('user_favorites')
          .update({ custom_end: customEndA }).eq('user_id', user.id).eq('artist_id', aId),
        supabase.schema('ultra').from('user_favorites')
          .update({ custom_start: customStartB }).eq('user_id', user.id).eq('artist_id', bId),
      ])
      if (e1) console.error('[my-settime] update customEnd error:', e1)
      if (e2) console.error('[my-settime] update customStart error:', e2)
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: 'var(--bg-primary)' }}>
      ...
      {!isEmpty && view === 'grid' && (
        <>
          <div className="md:hidden flex border-b shrink-0 overflow-x-auto" style={{ borderColor: 'var(--border)' }}>
            {STAGES.map((stage) => {
                ...
                <div className="sticky left-0 shrink-0" style={{ width: 44, background: 'var(--bg-primary)', zIndex: 15 }}>
                  ...
                  <div style={{ position: 'relative', height: TOTAL_HEIGHT }}>
                    {HOUR_LABELS.map((label, i) => (
                      <span key={label} style={{ position: 'absolute', top: i * 108 - 7, right: 6, fontSize: 9, fontWeight: 700, color: 'var(--text-secondary)', letterSpacing: '0.02em', lineHeight: 1 }}>
                        {label}
                      </span>
                    ))}
                  </div>
      ...
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center" style={{ background: 'rgba(10,10,15,0.7)', zIndex: 50 }}>
          <span style={{ color: 'var(--cyan)', fontSize: 12, fontWeight: 700, letterSpacing: '0.1em' }}>CARGANDO...</span>
        </div>
      )}
